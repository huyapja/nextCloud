...
# 		),
# 		"request_count_by_ip": get_nginx_request_by_(name, "count", timezone, timespan, timegrain,max_no_of_paths),
...
# 	return {
# 		# "data": [{"value": r.max, "date": r.date} for r in request_data], ham cua chuan
# 		"data": [{"value": getattr(r, "max", None), "date": getattr(r, "date", None)} for r in request_data], # ham fix
...
# 	print("response get usage: ", response.text)
...
# 	print("get_current_cpu_usage called with", site)
# 	try:
# 		log_server = frappe.db.get_single_value("Press Settings", "log_server")
# 		if not log_server:
# 			return 0
...
# 		url = f"https://{log_server}/elasticsearch/filebeat-*/_search"
# 		password = get_decrypted_password("Log Server", log_server, "kibana_password")
# 		print("Log server URL:", url)
# 		print("Log server password:", password)
...
# 	print("get_current_cpu_usage_for_sites_on_server called with", server)
# 	result = {}
...
# 	# return resulttry:
# 		try:
# 			for row in data["aggregations"]["0"]["buckets"]:
# 				site = row["key"]
# 				metric = row["usage"]["counter"]["top"]
# 				if metric:
# 					result[site] = metric[0]["metrics"]["json.request.counter"]
...
# 			frappe.log_error(
# 				title="Analytics Parse Aggregations Error",
# 				message=f"Lỗi khi xử lý dữ liệu trả về từ API.\nError: {str(e)}\nData: {data}"
# 			)
# 			return []
# 		return result
...
# 	print("request_logs called with", name, timezone, date, sort, start)
# 	log_server = frappe.db.get_single_value("Press Settings", "log_server")
# 	if not log_server:
# 		return []
...
class StackedGroupByChart:
	...
	def get_histogram_chart(
		...
		print('debug get_histogram_chart')
		hist_bucket: HistBucket
		for hist_bucket in path_bucket.histogram_of_method.buckets:
			label = get_datetime(hist_bucket.key_as_string)
			if label not in labels:
	...
	def get_stacked_histogram_chart(self):
		...
		path_bucket: PathBucket
		print('debug: aggs',aggs.method_path)
		for path_bucket in aggs.method_path.buckets:
			datasets.append(self.get_histogram_chart(path_bucket, labels))
		...
		if self.normalize_slow_logs:
			datasets = normalize_datasets(datasets) 
		labels = [label.replace(tzinfo=None) for label in labels]
		# datasets.append({"path": "Other", "values": [20] * len(labels), "stack": "path"})
...
class SlowLogGroupByChart(StackedGroupByChart):
	...
	def run(self):
		print('debug',self.name , ' - ',self.database_name, ' - ', self.resource_type)
		# if not self.database_name and ResourceType(self.resource_type) is ResourceType.SITE: 
		# 	return {"datasets": [], "labels": []}
		res = super().run() 
		if ResourceType(self.resource_type) is not ResourceType.SERVER:
...
def get(name, timezone, duration="7d"):
	timespan, timegrain = TIMESPAN_TIMEGRAIN_MAP[duration]
	print("debug:",name,timezone,duration,timespan,timegrain)
	request_data = get_usage(name, "request", timezone, timespan, timegrain)
	uptime_data = get_uptime(name, timezone, timespan, timegrain)
	...
	return {
		"usage_counter": [ {
                "date": "2025-06-09 21:41:48.374000+07:00",
                "value": 1.0
            }],
			"request_count": [ {
                "date": "2025-06-09 21:41:48.374000+07:00",
...
def get_uptime(site, timezone, timespan, timegrain):
	print('get_uptime',site,timezone,timespan,timegrain)
	monitor_server = frappe.db.get_single_value("Press Settings", "monitor_server")
	if not monitor_server:
		return []
